...
func describeOp(op executor.Operator) (name string, children []executor.Operator) {
	...
	case *executor.Filter:
		return "Filter (WHERE ...)", getChild(o)

	case *executor.Project:
		cols := o.Schema().ColNames()
		return fmt.Sprintf("Project [%s]", strings.Join(cols, ", ")), getProjectChild(o)

	case *executor.Sort:
		return "Sort (ORDER BY ...)", getSortChild(o)

	case *executor.Limit:
		return "Limit", getLimitChild(o)

	case *executor.Aggregate:
		return "Aggregate (GROUP BY + agg funcs)", getAggChild(o)
	...
	case *executor.Distinct:
		return "Distinct", getDistinctChild(o)
